...
void conf_mybatteria(char *devicev, char *devicea, int R1, int R2, int diodo, int Aref, int periodo) {
	...
	sprintf(batteria_conf.devicev, "%s", devicev);
	sprintf(batteria_conf.devicea, "%s", devicea);
...
int init_mybatteria(mybatteria_t *oldvalue) {
	while(((batteria_conf.fd_batteriav = fopen(batteria_conf.devicev,"r"))==NULL) && ((batteria_conf.fd_batteriaa = fopen(batteria_conf.devicea,"r"))==NULL)) {
	...
	oldvalue->stato = (oldvalue->capacita * 10000)/oldvalue->capacita_nom;
...
void deinit_mybatteria() {
	...
	fclose(batteria_conf.fd_batteriaa);
}
...
void write_mybatteria_sensori(mybatteria_t *mybatteria){
	...
	batter.corrente = mybatteria->corrente;
...
mybatteria_t  misura_mybatteria(mybatteria_t oldvalue) {
	...
	if(oldvalue.volt == 0) {
		...
		newvalue.corrente = readBatteryCurrent();
		return(newvalue);
	}
...
*/
	...
	if (batteryCurrent > oldvalue.corrente_max) {
		highBatteryEvent();
	}
	...
	newvalue.corrente = batteryCurrent;

	newvalue.capacita = oldvalue.capacita - (batteryCurrent * batteryVoltage * batteria_conf.periodo/1000000);

	newvalue.stato = (newvalue.capacita * 10000)/newvalue.capacita_nom;
	return(newvalue);
}
...
int readBatteryCurrent() {
	int n, res;
	//fread(&n, sizeof(int), 1, fd_batteria);
	res = fscanf(batteria_conf.fd_batteriaa, "%d", &n);
	rewind(batteria_conf.fd_batteriaa);
