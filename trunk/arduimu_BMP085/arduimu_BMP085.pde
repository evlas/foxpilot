...
// Version 1.9 includes support for BMP058 absolute pressure sensor
...
#define MAGNETIC_DECLINATION 2.1    // corrects magnetic bearing to true north

/* Support for optional barometer (1 enabled, 0 dissabled) */
#define USE_BAROMETER 2 	// use 1 if you want to get altitude using the optional absolute pressure sensor
...
void setup() {
	...
	APM_Compass.Init();	// I2C initialization
        APM_Compass.SetOrientation(APM_COMPASS_SPARKFUN_COMPONENTS_UP_PINS_RIGHT);
        APM_Compass.SetOffsets(0,0,0);  // set offsets to account for surrounding interference
        APM_Compass.SetDeclination(ToRad(MAGNETIC_DECLINATION));  // set local difference between magnetic north and true north
 	debug_handler(3);
#endif
...
#endif
#if USE_BAROMETER == 2
		BMP085.Read();    // Read I2C
		press_filt = (BMP085.Press + 2l * press_filt) / 3l;		//Light filtering
		//temperature = (temperature * 9 + temp_unfilt) / 10;    We will just use the ground temp for the altitude calculation
#endif
		break;
...
#if USE_BAROMETER == 2
	BMP085.Read();
	press_gnd = BMP085.Press;
	temperature = BMP085.Temp;
	delay(20);
#endif
...
#if USE_BAROMETER == 2
			BMP085.Read();
			press_gnd = (press_gnd * 9l + BMP085.Press) / 10l;
			temperature = (temperature * 9 + BMP085.Temp) / 10;
			delay(20);
#endif
			digitalWrite(7,HIGH);
			digitalWrite(6,LOW);
